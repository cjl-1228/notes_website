...
function CourseAllHanzi(){
    /* var character = document.querySelector('.js-char').value; */
    var statAllHanzi = ['舟','灑','載','愁','讓','嘆','忌','妒','語','訣']; //此課程生字 
    for (let index = 0; index < statAllHanzi.length; index++) {
        HanziWriter.loadCharacterData(statAllHanzi[index]).then(function(charData) {
            var target = document.getElementById('tmp-svg');//存放按下確定後的所有漢字 div
            var svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
            svg.style.width = '100px';
            svg.style.height = '100px';
